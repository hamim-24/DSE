...
const ArabicPage: React.FC = () => {
  ...
  // Filter courses for arabic category
  const arabicCourses = courses
    .filter((course) => {
      // Base category filter
      if (course.category !== "arabic") return false
    ...
    <div className="py-8">
      <div className="container mx-auto px-4">
        ...
        <div className="flex items-center mb-6">
          ...
          <h1 className="text-3xl font-bold">আরবি</h1>
        </div>

        {/* Banner */}
        <div className="bg-gradient-to-r from-lime-600 to-green-600 rounded-xl shadow-md p-8 mb-8 text-white relative overflow-hidden">
          ...
          <div className="relative z-10">
            <h2 className="text-2xl font-bold mb-2">আরবি কোর্সসমূহ</h2>
            <p className="text-lg opacity-90 max-w-2xl">
              আরবি ভাষা, ব্যাকরণ, উচ্চারণ সম্পর্কে জানুন। আমাদের ইন্টারেক্টিভ কোর্সগুলো আপনাকে আরবি ভাষার দক্ষতা বাড়াতে সাহায্য করবে।
            </p>
          </div>
        </div>
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-4">
          <div>
            <p className="text-gray-600">মোট {arabicCourses.length} টি কোর্স পাওয়া গেছে</p>
          </div>
          <div className="mt-4 md:mt-0 flex items-center gap-2">
            <div className="relative w-full md:w-72">
              <input
                ...
                className="w-full px-4 py-2 pl-10 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-lime-500"
            ...
            <div className="flex flex-wrap gap-2 items-center">
              ...
              {activeFilters.map((filter, index) => (
                <motion.div
                  ...
                  className="flex items-center gap-1 px-2 py-1 bg-lime-50 border border-lime-200 rounded-md text-sm"
                >
                  <span>{filter}</span>
              ...
              <button onClick={clearAllFilters} className="text-sm text-lime-600 hover:text-lime-800">
                সব মুছুন
              </button>
            </div>
          </div>
        ...
        {arabicCourses.length > 0 ? (
            ...
            {arabicCourses.map((course) => (
              ...
              >
                <div className="relative h-48 overflow-hidden">
                  ...
                  />
                  <div className="absolute top-3 right-3 px-2 py-1 rounded-md text-xs font-medium bg-lime-600 text-white">
                    {levels.find((l) => l.id === course.level)?.name}
                  </div>
                </div>

                <div className="p-5">
                  <h3 className="font-bold text-lg mb-3 line-clamp-2 hover:text-lime-600 transition-colors">
                    {course.title}
                  </h3>
                  ...
                  {course.progress > 0 ? (
                    <div>
                      ...
                      <div className="w-full bg-gray-200 rounded-full h-2">
                        <div className="h-2 rounded-full bg-lime-600" style={{ width: `${course.progress}%` }}></div>
                      </div>
                      <button
                        className="mt-4 w-full bg-lime-100 hover:bg-lime-200 text-lime-800 py-2 rounded-md font-medium transition-colors"
                        onClick={(e) => {
                          e.stopPropagation()
                    ...
                    <button
                      className="w-full bg-lime-600 hover:bg-lime-700 text-white py-2 rounded-md font-medium transition-colors"
                      onClick={(e) => {
                        e.stopPropagation()
                        handleStartCourse(course.id)
                      }}
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="py-16 text-center">
            ...
            <button
              className="bg-lime-600 hover:bg-lime-700 text-white px-4 py-2 rounded-md font-medium transition-colors"
